	...
	draw() {
		...
		const drawXAxis = () => {
			...
			for (let i = 0; i < count; i++) {
				...
				const valueText =
					value >= 1000 ? numberformat.formatShort(value) : round(value, 1);
		...
		const drawLabels = () => {
			...
			const labelsWidth =
				this.#data
					.flatMap((d) => ctx.measureText(d.label).actualBoundingBoxRight)
					.reduce((sum, m) => sum + m) +
			...
			for (let i = 0; i < this.#data.length; i++) {
				...
				labelCurrentOffset += measure.actualBoundingBoxRight + 2.5 * gap;
			}
		};
