...
*/
  ...
  getSegmentSolution(
    ...
    let decision = 0;
    let permuationLength = permutationArr[0].length;
    let bestSolution = [
      permutationArr[0],
    ...
    for (let i = 0; i < permutationArr.length; i++) {
      ...
      for (let j = 0; j < permutationArr[i].length; j++) {
        ...
        if (decisions[permutationArr[i][j]] === "wait") {
          ...
          let waitSaveUpTime =
            (objective.value - currentGameState.cookies) / currentGameState.cps;
      ...
      if (i === 0) {
        ...
        if (testPrint === 1) {
          console.log(`Temp BEST segment solution:`, permutationArr[i]);
        }
        continue;
      }
  ...
  getBruteForceSegmentedSolution(objective, decisions) {
    ...
    if (objective.type === "production") {
      for (let i = 0; endMarker !== decisions.length /*i < 2*/; i++) {
        ...
        if (
          referenceGameState.buildingCpS >= segmentSolutionData[1].buildingCpS
        ) {
          throw new Error(
        ...
        console.log("referenceGameStateBEFORE", referenceGameState.buildingCpS);
        segmentSolution = segmentSolutionData[0];
        referenceGameState = segmentSolutionData[1].copy();
        console.log("referenceGameStateAFTER", referenceGameState.buildingCpS);
        if (
          referenceGameState.buildingCpS !== segmentSolutionData[1].buildingCpS
        ) {
          throw new Error(
